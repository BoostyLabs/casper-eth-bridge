...
// Error is a default error type for bnb connector cli.
var Error = errs.Class("bnb connector cli")
...
var (
	rootCmd = &cobra.Command{
		Use:   "connector",
		Short: "cli for interacting with bnb connector project",
	}
...
func cmdRun(cmd *cobra.Command, args []string) (err error) {
	...
	err = godotenv.Overload("./configs/.bnb.env")
	if err != nil {
		log.Error("could not load bnb config: %v", Error.Wrap(err))
	...
	{ // BNB smart chain server setup.
		// connect client to default http connection node.
		nodeClient, err := ethclient.Dial(config.Config.NodeAddress)
		if err != nil {
			return Error.Wrap(err)
		}

		instance, err := bridge.NewBridge(config.Config.BridgeContractAddress, nodeClient)
		if err != nil {
			return Error.Wrap(err)
		}

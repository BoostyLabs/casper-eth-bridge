...
// CreateMasterDB creates a new bridge.DB for testing.
func CreateMasterDB(ctx context.Context, name string, category string, index int, dbInfo Database) (db bridge.DB, err error) {
	if dbInfo.URL == "" {
...
// CreateMasterDBOnTopOf creates a new bridge.DB on top of an already existing
// temporary database.
func CreateMasterDBOnTopOf(tempDB *tempdb.TempDatabase) (bridge.DB, error) {

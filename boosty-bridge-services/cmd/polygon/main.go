...
import (
	...
	polygon_server "github.com/BoostyLabs/casper-eth-bridge/boosty-bridge-services/chains/server"
...
// Error is a default error type for polygon connector cli.
var Error = errs.Class("polygon connector cli")
...
func cmdRun(cmd *cobra.Command, args []string) (err error) {
	...
	err = godotenv.Overload("./configs/.polygon.env")
	if err != nil {
		log.Error("could not load polygon config: %v", Error.Wrap(err))
	...
	{ // Server setup.
		...
		server = polygon_server.NewServer(ctx, log, registerServer, config.GrpcServerAddress, config.ServerName)
	}
